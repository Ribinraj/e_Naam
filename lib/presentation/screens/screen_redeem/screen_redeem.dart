...
class _ScreenHistoryPageState extends State<ScreenRedeemPage> {
  ...
  Future<void> _onRefresh() async {
    ...
    // Refresh categories (listener will take care of products)
    context.read<FetchCategoriesBloc>().add(FetchCategoriesInitialeEvent());
    // Also re-fetch products for the currently selected category to feel instant
    if (selectedCategoryId != null) {
      context.read<FetchProductBloc>().add(
            FetchProductButtonClickEvent(categoryId: selectedCategoryId!),
          );
    }
        ...
        listeners: [
          BlocListener<FetchCategoriesBloc, FetchCategoriesState>(
            listener: (context, state) {
              if (state is FetchCategoriesSuccessState &&
                  state.categories.isNotEmpty) {
                // If current selection is gone or null, select first; otherwise keep selection
                final hasCurrent =
                ...
                final String effectiveCategoryId = hasCurrent
                    ? selectedCategoryId!
                ...
                // Always fetch products for the effective category after a categories refresh
                context.read<FetchProductBloc>().add(
                      FetchProductButtonClickEvent(
